...
async function chaySeedDuLieu() {
  try {
    ...
    await InventoryLog.create({
      ...
      type: "Import",
      quantity: 12,
    }); // Nhập ít hơn minQuantity (30) để test cảnh báo

    // Xuất bớt hàng ra để số lượng thay đổi sinh động
    await InventoryLog.create({
      productId: sanPham1._id,
      userId: taiKhoanStaff._id,
      type: "Export",
    ...
    // Cập nhật số lượng tồn kho cuối cùng (Quantity = Nhập - Xuất)
    console.log("[SEED] Đang đồng bộ số lượng sản phẩm tồn kho thực tế...");
    await Product.findByIdAndUpdate(sanPham1._id, { quantity: 20 }); // 25 - 5
    await Product.findByIdAndUpdate(sanPham2._id, { quantity: 50 }); // 60 - 10
    await Product.findByIdAndUpdate(sanPham3._id, { quantity: 25 }); // 40 - 15
    await Product.findByIdAndUpdate(sanPham4._id, { quantity: 15 }); // 15 - 0
    await Product.findByIdAndUpdate(sanPham5._id, { quantity: 30 }); // 30 - 0
    await Product.findByIdAndUpdate(sanPham6._id, { quantity: 12 }); // 12 - 0 (Sẽ báo đỏ yếu kho)
